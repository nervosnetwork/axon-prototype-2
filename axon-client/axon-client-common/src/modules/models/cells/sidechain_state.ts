...
export class SidechainState implements CellInputType, CellOutputType, CellDepType {
    ...
    constructor(capacity: bigint, chainId: bigint, version: bigint, latestBlockHeight: bigint, latestBlockHash: string, committedBlockHeight: bigint, committedBlockHash: string, status: bigint, outPoint: OutPoint) {
    ...
    static fromCell(cell: Cell): SidechainState | null {
        ...
        let latestBlockHeight = leHexToBigIntUint128(data.substring(4, 36))
        let latestBlockHash = data.substring(36, 100)
        let committedBlockHeight = leHexToBigIntUint128(data.substring(100, 132))
        let committedBlockHash = data.substring(132, 196)
        let status = leHexToBigIntUint8(data.substring(196, 198))

        let outPoint = cell.out_point!

        return new SidechainState(capacity, chainId, version,
            latestBlockHeight, latestBlockHash, committedBlockHeight,
            committedBlockHash, status, outPoint)
    }

    static default(): SidechainState {
        return new SidechainState(0n, 0n, 0n, 0n, ``, 0n,
            ``, 0n, defaultOutPoint())
    }
    ...
    toCellOutputData(): string {
        return `0x${
            ...
            remove0xPrefix(Uint128BigIntToLeHex(this.latestBlockHeight))}${
            remove0xPrefix(this.latestBlockHash)}${
            remove0xPrefix(Uint128BigIntToLeHex(this.committedBlockHeight))}${
            remove0xPrefix(this.committedBlockHash)}`
    }
    ...
    static fromJSON(source: Object): SidechainState {
