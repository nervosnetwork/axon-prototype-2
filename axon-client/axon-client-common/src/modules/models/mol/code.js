...
export class CodeCellLockArgs {
  ...
  validate(compatible = false) {
    assertDataLength(this.view.byteLength, CodeCellLockArgs.size());
    this.getLockArg().validate(compatible);
  }
  static size() {
    return 0 + PubKeyHash.size();
  }
}

export function SerializeCodeCellLockArgs(value) {
  const array = new Uint8Array(0 + PubKeyHash.size());
