...
export class SidechainFeeCellLockArgs {
  ...
  getSurplus() {
    return new Uint128(this.view.buffer.slice(0 + ChainId.size(), 0 + ChainId.size() + Uint128.size()), { validate: false });
  }

  validate(compatible = false) {
    assertDataLength(this.view.byteLength, SidechainFeeCellLockArgs.size());
    this.getChainId().validate(compatible);
    this.getSurplus().validate(compatible);
  }
  static size() {
    return 0 + ChainId.size() + Uint128.size();
  }
}

export function SerializeSidechainFeeCellLockArgs(value) {
  const array = new Uint8Array(0 + ChainId.size() + Uint128.size());
  const view = new DataView(array.buffer);
  array.set(new Uint8Array(SerializeChainId(value.chain_id)), 0);
  array.set(new Uint8Array(SerializeUint128(value.surplus)), 0 + ChainId.size());
  return array.buffer;
}
