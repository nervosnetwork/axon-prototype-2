...
export class SidechainRegistryCell {
  ...
  validate(compatible = false) {
    ...
    new ChainIdList(this.view.buffer.slice(offsets[0], offsets[1]), { validate: false }).validate();
  }
...
export function SerializeSidechainRegistryCell(value) {
  const buffers = [];
  buffers.push(SerializeChainIdList(value.chain_ids));
