...
export class CollatorRefreshTaskTransformation implements Transformation {
  ...
  constructor(depGlobalConfig: GlobalConfig, depConfig: SidechainConfig, inputCode: Code, inputTasks: Array<Task>) {
  ...
  toCellInput(): Array<CKBComponents.CellInput> {
    return [this.inputCode.toCellInput(), ...this.inputTasks.map((task) => task.toCellInput())];
  }

  toCellOutput(): Array<CKBComponents.CellOutput> {
    return [this.inputCode.toCellOutput(), ...this.inputTasks.map((task) => task.toCellOutput())];
  }

  toCellOutputData(): Array<string> {
    return [this.inputCode.toCellOutputData(), ...this.inputTasks.map((task) => task.toCellOutputData())];
  }
