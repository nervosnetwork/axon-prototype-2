...
fn bootstrap(
    ...
    let (builder, lock_script) = load_script(builder, context, pubkey_hash, chain_bitmap);
...
#[test]
fn test_sign_with_correct_key_but_busy() {
    // generate key pair
    let privkey = Generator::random_privkey();
    let pubkey = privkey.pubkey().expect("pubkey");
    let pubkey_hash = blake160(&pubkey.serialize()).to_vec();
...
#[test]
fn test_sign_with_correct_key_and_free() {
    // generate key pair
    let privkey = Generator::random_privkey();
    let pubkey = privkey.pubkey().expect("pubkey");
    let pubkey_hash = blake160(&pubkey.serialize()).to_vec();
