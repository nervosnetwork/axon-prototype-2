...
use crate::{
    ...
    molecule::{
        ...
        witness::common_submit_jobs::{
            CommonSubmitJobsWitnessBuilder, CommonSubmitJobsWitnessReader, ExistedCommittedCheckerInfoBuilder,
...
impl Default for CommonSubmitJobsWitness {
    fn default() -> Self {
        Self {
            chain_id:           0,
            commit:             Vec::default(),
...
impl FromRaw for CommonSubmitJobsWitness {
    fn from_raw(witness_raw_data: &[u8]) -> Option<CommonSubmitJobsWitness> {
        let reader = CommonSubmitJobsWitnessReader::from_slice(witness_raw_data).ok()?;

        Self::from_reader(reader)
    }
}

impl Serialize for CommonSubmitJobsWitness {
    type RawType = Vec<u8>;
    ...
    fn serialize(&self) -> Self::RawType {
        ...
        self.as_builder()
            .write(&mut buf)
            .expect("Unable to write buffer while serializing CommonSubmitJobsWitness");
