...
impl ::core::default::Default for CollatorSubmitTasksWitness {
    fn default() -> Self {
        let v: Vec<u8> = vec![
            140, 0, 0, 0, 8, 0, 0, 0, 132, 0, 0, 0, 28, 0, 0, 0, 32, 0, 0, 0, 36, 0, 0, 0, 68, 0, 0, 0, 100, 0, 0, 0, 116, 0, 0, 0, 0, 0,
...
impl CollatorSubmitTasksWitness {
    ...
    pub fn common(&self) -> CommonSubmitJobsWitness {
        let slice = self.as_slice();
        let start = molecule::unpack_number(&slice[4..]) as usize;
        if self.has_extra_fields() {
            let end = molecule::unpack_number(&slice[8..]) as usize;
            CommonSubmitJobsWitness::new_unchecked(self.0.slice(start..end))
        } else {
            CommonSubmitJobsWitness::new_unchecked(self.0.slice(start..))
        }
    }
...
impl molecule::prelude::Entity for CollatorSubmitTasksWitness {
    ...
    fn as_builder(self) -> Self::Builder {
        Self::new_builder().common(self.common())
    }
}
#[derive(Clone, Copy)]
...
impl<'r> CollatorSubmitTasksWitnessReader<'r> {
    ...
    pub fn common(&self) -> CommonSubmitJobsWitnessReader<'r> {
        let slice = self.as_slice();
        let start = molecule::unpack_number(&slice[4..]) as usize;
        if self.has_extra_fields() {
            let end = molecule::unpack_number(&slice[8..]) as usize;
            CommonSubmitJobsWitnessReader::new_unchecked(&self.as_slice()[start..end])
        } else {
            CommonSubmitJobsWitnessReader::new_unchecked(&self.as_slice()[start..])
        }
    }
}
impl<'r> molecule::prelude::Reader<'r> for CollatorSubmitTasksWitnessReader<'r> {
    ...
    fn verify(slice: &[u8], compatible: bool) -> molecule::error::VerificationResult<()> {
        ...
        CommonSubmitJobsWitnessReader::verify(&slice[offsets[0]..offsets[1]], compatible)?;
        Ok(())
    }
}
#[derive(Debug, Default)]
pub struct CollatorSubmitTasksWitnessBuilder {
    pub(crate) common: CommonSubmitJobsWitness,
}
impl CollatorSubmitTasksWitnessBuilder {
    pub const FIELD_COUNT: usize = 1;

    pub fn common(mut self, v: CommonSubmitJobsWitness) -> Self {
        self.common = v;
        self
    }
}
impl molecule::prelude::Builder for CollatorSubmitTasksWitnessBuilder {
    ...
    fn expected_length(&self) -> usize {
        molecule::NUMBER_SIZE * (Self::FIELD_COUNT + 1) + self.common.as_slice().len()
    }
    ...
    fn write<W: ::molecule::io::Write>(&self, writer: &mut W) -> ::molecule::io::Result<()> {
        ...
        offsets.push(total_size);
        total_size += self.common.as_slice().len();
        writer.write_all(&molecule::pack_number(total_size as molecule::Number))?;
        for offset in offsets.into_iter() {
            writer.write_all(&molecule::pack_number(offset as molecule::Number))?;
        }
        writer.write_all(self.common.as_slice())?;
        Ok(())
    }
